...
// Added Function 6/9/2002 - tmg
BOOL CRegistry::EnumerateAllSubKeys(CStringArray & arySubKeys) 
{ 
/* Call this function to get the subkeys.
    ...
    // get the name of each child key and add it to the array. 
    arySubKeys.RemoveAll();
...
BOOL CRegistry::WriteString(CString strName, CString strValue)
{
	...
	_tcscpy(sz, LPCTSTR(strValue));

	if (::RegOpenKeyEx(m_hRootKey, LPCTSTR(m_strCurrentPath), 0,
		KEY_WRITE, &hKey) != ERROR_SUCCESS) return FALSE;
	
	if (::RegSetValueEx(hKey, LPCTSTR(strName), 0,
		REG_SZ, (LPBYTE)sz, (DWORD) (_tcslen(sz) + 1))
		 != ERROR_SUCCESS) bSuccess = FALSE;
