...
public:
	...
	double		m_minX;		// min time
	double		m_maxX;		// max time
	int			m_iTimeMode;	// axis grid and legend interval index

	double		m_ticksPerPixel;

	timeaxis()
	{
		m_szTitle = "Time";
		m_minX = 0;
		m_maxX = MY_SIZE * (1.0 / MY_RATE);
		m_iTimeMode = 0;
		m_ticksPerPixel = 1;
	}
};
...
public:
	...
	void		MoveWindow(CRect &Rect, CWnd *pParent);
	...
	virtual void SetBXRange(double from, double to, BOOL bMove = TRUE);
	...
	virtual void ResetScale(int size, int rate);
