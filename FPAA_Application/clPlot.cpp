...
//* Author:			Jan Vidar Berger
//*******************************************************************************************************/
void serie::AddPoint(double &x, double &y)
{
	if (m_lNoValues > 0)
	...
	m_pvalues[m_lend].x = x;
	m_pvalues[m_lend].y = y;
	m_lNoValues++;
	m_lend++;
...
clPlot::clPlot()
{
	...
	m_lMaxDataPrSerie = INTEGER_MAX_VALUE;
	m_lMaxDataTotal = MAXSERIES * m_lMaxDataPrSerie;
	m_dNoData = 0.0;

	m_dzoom = 1.0;

	lArraySize = MY_SIZE;			// only points with differebt x,y will be put into the array

	pLineArray = new CPoint[lArraySize];
	SetBXRange(0, MY_SIZE * (1.0 / MY_RATE), FALSE);
...
void clPlot::ComputeRects(BOOL bInitialization)
{
	...
	//	if(bInitialization)
	//	{
	m_timeaxis.m_ticksPerPixel = ((double)(m_timeaxis.m_maxX - m_timeaxis.m_minX)) / (double)m_plotRect.Width();
...
void clPlot::OnPaint()
{
	...
	CCMemDC  pdc(&dc);  // non flickering painting
...
void clPlot::DrawSerie(CDC *dc, int s)
{
	...
	while (bMore) {
		...
		ly = 0;
		while (y != m && !bDraw) {
			if (m_series[s].m_pvalues[y].y == m_dNoData)
			{
				bDraw = TRUE;
				bMore = TRUE;
			}
				...
				double x = m_series[s].m_pvalues[y].x;
				p.x = (int)(m_plotRect.left + ((x - m_timeaxis.m_minX) / m_timeaxis.m_ticksPerPixel));

				if (bRightAxis) {
					p.y = (int)(m_plotRect.bottom - ((m_series[s].m_pvalues[y].y - m_rightaxis.minrange) / m_rightaxis.m_dValuePrPixel));
				}
				else {
					p.y = (int)(m_plotRect.bottom - ((m_series[s].m_pvalues[y].y - m_leftaxis.minrange) / m_leftaxis.m_dValuePrPixel));
				}
...
{
	double yGrid = m_timeaxis.m_minX;
	long delta = (long)(10.0 + (long)(((m_timeaxis.m_ticksPerPixel))) * 10);
	...
	for (long sx = m_timeaxis.m_minX - diff; sx < m_timeaxis.m_maxX; sx += d10)
	{
		int off = 3;
		if ((long)sx % (long)delta == 0) {
			off = 5;
		}
		if (sx > m_timeaxis.m_minX) {
			int x = (int)(m_plotRect.left + ((sx - m_timeaxis.m_minX) / m_timeaxis.m_ticksPerPixel));
	...
	old = dc->SelectObject(&pen);
	while (yGrid <= m_timeaxis.m_maxX)
	{
		int x = (int)(m_plotRect.left + ((yGrid - m_timeaxis.m_minX) / m_timeaxis.m_ticksPerPixel));

		if (yGrid > m_timeaxis.m_minX && yGrid<m_timeaxis.m_maxX) {
			dc->MoveTo(CPoint(x, m_plotRect.bottom - 1));
			dc->LineTo(CPoint(x, m_plotRect.top + 1));
		}
...
//*******************************************************************************************************/
BOOL clPlot::AddPoint(int serie, double &x, double &y)
{
	if (m_series[serie].m_lNoValues < m_lMaxDataPrSerie) {
		m_series[serie].AddPoint(x, y);
		if (m_bAutoScrollX && x > m_timeaxis.m_maxX) {
			//double span = m_timeaxis.m_maxX - m_timeaxis.m_minX;
			//double minval = x - span;

			SetBXRange(0, x + (MY_SIZE * (1.0 / MY_RATE)), FALSE);
...
//*******************************************************************************************************/
//*******************************************************************************************************/
void clPlot::SetBXRange(double min, double max, BOOL bMove)
{
	m_timeaxis.m_minX = min;
	m_timeaxis.m_maxX = max;
	if (!bMove) {
		m_timeaxis.m_ticksPerPixel = ((double)(m_timeaxis.m_maxX - m_timeaxis.m_minX)) / (double)m_plotRect.Width();
	}
}
...
void clPlot::ResetScale(int size, int rate)
{
	SetBXRange(0, size * (1.0 / rate), FALSE);
	lArraySize = size;
	pLineArray = new CPoint[lArraySize];
}

void clPlot::SetArraySize(int size)
{
	lArraySize = size;
	pLineArray = new CPoint[lArraySize];
}
...
//*******************************************************************************************************/
//*******************************************************************************************************/
void clPlot::MoveWindow(CRect & Rect, CWnd *pParent)
{
	m_ctlRect = Rect;
	pParent->ClientToScreen(m_ctlRect);
	pParent->ScreenToClient(m_ctlRect);
	ComputeRects(TRUE);
